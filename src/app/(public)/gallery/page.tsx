...
export default async function GalleryPage() {
  ...
  return (
    <main className="mx-auto max-w-content px-6 py-16">
      <h1 className="font-display text-3xl font-semibold text-slate-900">Gallery</h1>
        ...
        {posts.map((post) =>
          post.imageUrl ? (
            <figure key={post.id} className="group overflow-hidden rounded-lg">
              ...
              />
              <figcaption className="mt-2 text-sm text-slate-600">{post.title}</figcaption>
            </figure>
          ) : null,
        )}
        {posts.length === 0 && <p className="text-slate-500">No gallery images yet.</p>}
      </div>
    </main>
  );
}
