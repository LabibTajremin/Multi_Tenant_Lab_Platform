...
function MemberCard({ member, accent }: { member: Member; accent: ReturnType<typeof accentClasses> }) {
  return (
    ...
    >
      ...
      {member.bio && <p className="mt-1 line-clamp-4 text-sm text-slate-600 dark:text-slate-400">{member.bio}</p>}
...
export default async function PeoplePage() {
  ...
  return (
    <main className="mx-auto max-w-content px-6 py-16">
      <h1 className="font-display text-3xl font-semibold text-slate-900 dark:text-slate-100">People</h1>

      {sections.length === 0 && <p className="mt-6 text-slate-600 dark:text-slate-400">No team members listed yet.</p>}

      <div className="mt-10 space-y-14">
        {sections.map((section) =>
          section.position === 'Alumnus' ? (
            <details key={section.position} className="group">
              <summary className="cursor-pointer font-display text-xl font-semibold text-slate-900 dark:text-slate-100">
            ...
            <div key={section.position}>
              <h2 className="font-display text-xl font-semibold text-slate-900 dark:text-slate-100">{SECTION_LABELS[section.position]}</h2>
              <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {section.members.map((member) => (
                  <MemberCard key={member.id} member={member} accent={accent} />
                ))}
