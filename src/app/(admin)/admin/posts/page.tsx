  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl font-semibold text-slate-900">Funding &amp; Gallery</h1>
        <Link href="/admin/posts/new" className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
          + Add post
        </Link>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {posts.map((post) => (
              <tr key={post.id}>
                ...
                <td className="px-4 py-3 capitalize text-slate-700">{post.postType}</td>
                <td className="px-4 py-3">
                  <StatusBadge status={post.status} />
                </td>
                <td className="px-4 py-3 text-right space-x-3">
                  {canEditContent(user, post) && (
                    <Link href={`/admin/posts/${post.id}/edit`} className="text-slate-700 hover:underline">
                      Edit
                    </Link>
                  )}
